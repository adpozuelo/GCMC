...
__device__ precision __lennard_jones__(const precision r2, const unsigned short nit, const precision *al,
                                       const precision *bl2)
{
...
__global__ void __binary_reduction__(unsigned int *natoms_nsp, precision *g_idata, precision *g_odata)
{
...
__global__ void energy_gpu(precision *eng, unsigned int *natoms_nsp, unsigned short *itp, unsigned short *ptype,
                           precision *r, precision *side, precision *rc2, precision *al, precision *bl2,
                           precision *esrrc)
{
    __shared__ precision rx[NTHREAD], ry[NTHREAD], rz[NTHREAD];
    precision xi, yi, zi, rd2;
    unsigned int i = threadIdx.x + blockIdx.x * blockDim.x; // global thread index
    ...
    unsigned int ndmol = natoms_nsp[0] * NDIM;              // total size of global memory vector position
    precision energ = 0.0;
    precision rdd[NDIM];
    unsigned short nit;
    ...
    {
        int ml = m / nbl;
        int lim = (1 - ml) * NTHREAD + ml * rest; // calculate the limit in the block fit
        ...
        if (mtt <= ndmol) // if particle is valid get its position inside current block
        {
            rx[tx] = r[mtt];
            ry[tx] = r[mtt + 1];
            rz[tx] = r[mtt + 2];
        ...
        {
            for (int j = 0; j < lim; ++j)
            {                       // for every other particle inside the
...
__global__ void delta_energy_gpu(unsigned int ntest, precision *eng0, precision *eng1, precision *r_test,
                                 unsigned int *natoms_nsp, unsigned short *itp, unsigned short *ptype, precision *r,
                                 precision *side, precision *rc2, precision *al, precision *bl2, precision *esrrc)
{
...
extern "C" void gpu(Configuration *cxf, const unsigned short mode)
{
    ...
    { // move_atoms_gpu
        ...
        for (int i = 0; i < cxf->natoms; ++i)
        {
            ...
            delta_energy_gpu<<<nblock, NTHREAD>>>(ntest, e_before_dev, e_after_dev, r_test_dev, natoms_nsp_dev, itp_dev,
